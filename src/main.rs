...
pub fn main() -> io::Result<()> {
    ...
    loop {
        ...
        file.write_all(format!("player 1 : {}, at {:?}", client_1_addr, SystemTime::now()).as_bytes())?;

        let (mut client_2_socket, client_2_addr) = listener.accept()?;
        println!("Client 2 connected from {}", client_2_addr);
        file.write_all(format!("player 2 : {}, at {:?}", client_2_addr, SystemTime::now()).as_bytes())?;

        file.write_all(format!("game start!! at {:?} with player1({}) and player2({})", SystemTime::now(), client_1_addr, client_2_addr).as_bytes())?;
        ...
        file.write_all(format!("game ended at {:?} with player1({}) and player2({})", SystemTime::now(), client_1_addr, client_2_addr).as_bytes())?;
    }
